...
Validated on 2026-09-17 for v0.2.0-beta.3 (installed version 0.2.0b3).

## Automated checks

Node 22.23.1 on Linux: TypeScript checking, ESLint, and all 88 Vitest tests passed. Tests cover reference matching, the SQLite query, batching and cancellation, overlapping searches, panel behavior, and lifecycle cleanup. The dependency installation reported zero audit vulnerabilities.
...
Verified embedded and explicit-link references, exclusion of plain text and trashed notes/parents, filtering, directly opening the correct note in both native tab and window modes, keyboard dismissal, reopening, repeated shutdown, repeated startup without duplicate listeners, and menu operation after disabling and reenabling. The runner enforces profile/marker guards, fresh result files, a timeout, and process cleanup. The normal user profile was not modified. The integration harness also creates a real text annotation, asks Zotero to create a note from it, verifies that the default insertion omits the annotation identifier, and finds/opens it through the actual reader menu as a Possible match.
...
| --- | --- | --- | --- | --- |
| 1,000 | 100 | 1.807 seconds | -0.7 MiB | 10 / 10 |
| 10,000 | 1,000 | 94.176 seconds | 14.6 MiB | 100 / 100 |
